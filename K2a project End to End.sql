...
with TopCustomer as 
(
select Equiptmentname, TransYear, GrossAmount, Position
						
					FROM 
								(Select b.EquiptmentName,Datepart(Year,Transdate) as Transyear,GrossAmount,
								Dense_RANK () OVER (Partition by Datepart(Year,Transdate) order by GrossAmount desc) as Position
								fROM EquiptmentTransaction A
								INNER JOIN Equiptment b ON A.Equiptment_Sk = B.EquiptmentID) abu) 
        
								INSERT INTO @ResultTable 
								Select Top 10* from Topcustomer 
								Where @Transyear= Transyear and Position <= @NthValue	 
...
FROM
    EquiptmentTransaction A
	inner join Equiptment B ON B.EquiptmentID = A.Equiptment_Sk
...
--- created tables and function---
select * from FloatCategory
select * from Customer
select * from Equiptment
select * from EquiptmentTransaction
select * from Top10Customerlist(2019,10)
